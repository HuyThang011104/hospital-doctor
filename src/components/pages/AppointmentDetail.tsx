...
export default function AppointmentDetailPage({
  ...
  const [medicalRecord, setMedicalRecord] =
    useState<AppointmentMedicalRecord | null>(null);
  ...
  const recordId = useMemo(() => {
    ...
    if (
      ...
      String(medicalRecord.appointment_id) !== String(appointment.id)
    ) {
      return undefined;
    }
  ...
  useEffect(() => {
    ...
    const fetchRecordDetails = async () => {
      try {
        ...
        const { data: examinationsData, error: examinationsError } =
          await supabase
            .from("examination")
            .select("*")
  ...
  // --- KHỐI STATE & BIẾN CỦA BẠN (GIỮ NGUYÊN) ---
  ...
  // Hàm kiểm tra xem cuộc hẹn có thể chỉnh sửa được không
  const isAppointmentEditable = () => {
    const status = appointment.status;
    return status !== "Completed" && status !== "Cancelled";
  };

  // --- CHỨC NĂNG XÓA (MỚI THÊM) ---

  const handleDeleteLabTest = async (testId: number) => {
    if (!recordId) {
      toast.error("Hồ sơ y tế không được liên kết!");
      return;
    }
    try {
  ...
  const handleDeletePrescription = async (prescriptionId: number) => {
    ...
    try {
      ...
      setRecordPrescriptions(
        recordPrescriptions.filter((p) => p.id !== prescriptionId)
      );
      toast.success("Đơn thuốc đã được xóa thành công");
    } catch (error) {
      console.error("Lỗi xóa đơn thuốc:", error);
      toast.error("Failed to delete prescription. Vui lòng kiểm tra RLS!");
    }
  };

  // --- KHỐI HÀM DATABASE CỦA BẠN (GIỮ NGUYÊN) ---
  ...
  const handleCompleteAppointment = async () => {
    if (!diagnosis.trim() || !treatment.trim()) {
      toast.error(
        "Please complete the medical record before marking as completed"
      );
      return;
    }
